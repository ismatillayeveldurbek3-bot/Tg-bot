...
# CONFIG
# =====================================================
# Railway / Render / local deploy compatibility.
# Asosiy nom: BOT_TOKEN. Qo'shimcha nomlar eski loyihalar bilan moslik uchun.
...
BOT_TOKEN = get_env_any("BOT_TOKEN", "TOKEN", "TELEGRAM_BOT_TOKEN", "BOT_API_TOKEN", "API_TOKEN")
if not BOT_TOKEN:
    raise RuntimeError(
        "Telegram bot token topilmadi. Railway > Variables bo‘limiga "
        "BOT_TOKEN nomi bilan token qo‘shing. Masalan: BOT_TOKEN=123456:ABC..."
    )
...
def admin_kb(): return ik([[('📊 Dashboard','admin:dash')],[('👨‍🏫 O‘qituvchilar','admin:teachers'),('🏆 Reyting','admin:rating')],[('📢 Shikoyatlar','admin:complaints'),('💡 Takliflar','admin:suggestions')],[('📁 Export / Backup','admin:export'),('🧹 Tozalash','admin:clear')],[('⚙️ Sozlamalar','admin:settings')]])
...
def admin_settings_kb(): return ik([[('🏛 Kafedralar','settings:deps'),('📢 Shikoyatlar','settings:complaints')],[('🗳 Baholash','settings:voting'),('📊 Reyting','settings:rating')],[('🔐 Adminlar','settings:admins'),('📦 Backup','settings:backup')],[('⬅️ Admin','admin:menu')]])

CLEAR_TARGETS = {
    'users': ('Foydalanuvchilar', 'users'),
    'ratings': ('Baholar / ratinglar', 'ratings'),
    'complaints': ('Shikoyatlar', 'complaints'),
    'suggestions': ('Takliflar', 'suggestions'),
    'votes': ('Eski ovozlar', 'votes'),
    'teachers': ('O‘qituvchilar', 'teachers'),
    'departments': ('Kafedralar', 'departments'),
}

def admin_clear_kb():
    return ik([
        [('👥 Users','clear:ask:users'),('⭐ Ratings','clear:ask:ratings')],
        [('📢 Shikoyatlar','clear:ask:complaints'),('💡 Takliflar','clear:ask:suggestions')],
        [('🗳 Eski ovozlar','clear:ask:votes')],
        [('👨‍🏫 O‘qituvchilar','clear:ask:teachers'),('🏛 Kafedralar','clear:ask:departments')],
        [('⬅️ Admin','admin:menu')]
    ])

def clear_confirm_kb(target: str):
    return ik([[('✅ Ha, tozala', f'clear:do:{target}')],[('⬅️ Tozalash menyusi','admin:clear'),('❌ Bekor','admin:menu')]])
...
async def cancel_button(m: Message, state: FSMContext):
    ...
    if current in (DepartmentFSM.add_name.state, DepartmentFSM.edit_name.state, DepartmentFSM.sort_order.state):
        ret_settings = data.get('return_to_settings')
        await state.clear()
        if ret_settings:
            return await m.answer('🏛 <b>Kafedralar sozlamasi</b>', reply_markup=ik([[('⬅️ Kafedralar sozlamasi','settings:deps'),('⬅️ Sozlamalar','admin:settings')]]))
...
async def dep_add_save(m:Message,state:FSMContext):
    ...
    conn.execute("INSERT INTO departments(department_key,name,sort_order,created_at) VALUES(?,?,?,?)",(key,name,order,now_str())); conn.commit()
    data = await state.get_data()
    await state.clear()
    if data.get('return_to_settings'):
        return await m.answer("✅ Kafedra qo‘shildi.", reply_markup=ik([[('⬅️ Kafedralar sozlamasi','settings:deps'),('⬅️ Sozlamalar','admin:settings')]]))
...
async def dep_edit_save(m:Message,state:FSMContext):
    name=(m.text or '').strip()
    if len(name)<3: return await m.answer("Nom juda qisqa.", reply_markup=cancel_kb())
    data=await state.get_data(); conn.execute("UPDATE departments SET name=? WHERE department_key=?",(name,data['dkey'])); conn.commit(); dkey=data.get('dkey'); ret=data.get('return_to_settings'); await state.clear()
    if ret:
        return await m.answer("✅ Kafedra yangilandi.", reply_markup=ik([[('⬅️ Kafedra oynasi',f'settings:dep:{dkey}'),('⬅️ Sozlamalar','admin:settings')]]))
...
@dp.callback_query(F.data == 'settings:depadd')
@admin_required
async def settings_dep_add_start(call: CallbackQuery, state: FSMContext):
    await answer_cb(call)
    await state.update_data(return_to_settings=True)
    await state.set_state(DepartmentFSM.add_name)
    await ask_input(call, 'Yangi kafedra nomini kiriting:')

@dp.callback_query(F.data.startswith('settings:depedit:'))
@admin_required
async def settings_dep_edit_start(call: CallbackQuery, state: FSMContext):
    await answer_cb(call)
    dkey = call.data.split(':', 2)[2]
    await state.update_data(dkey=dkey, return_to_settings=True)
    await state.set_state(DepartmentFSM.edit_name)
    await ask_input(call, f"Hozirgi nom: <b>{safe(dep_name(dkey))}</b>\nYangi nomni kiriting:")

@dp.callback_query(F.data.startswith('settings:deptoggle:'))
@admin_required
async def settings_dep_toggle(call: CallbackQuery):
    await answer_cb(call)
    dkey = call.data.split(':', 2)[2]
    row = conn.execute('SELECT is_active FROM departments WHERE department_key=?', (dkey,)).fetchone()
    if not row:
        return await edit_or_send(call, 'Kafedra topilmadi.', ik([[('⬅️ Kafedralar sozlamasi','settings:deps')]]))
    new_value = 0 if row['is_active'] else 1
    conn.execute('UPDATE departments SET is_active=? WHERE department_key=?', (new_value, dkey))
    conn.commit()
    await settings_dep_detail(call)

@dp.callback_query(F.data.startswith('settings:dep:'))
@admin_required
async def settings_dep_detail(call: CallbackQuery):
    await answer_cb(call)
    dkey = call.data.split(':', 2)[2]
    d = conn.execute('SELECT * FROM departments WHERE department_key=?', (dkey,)).fetchone()
    if not d:
        return await edit_or_send(call, 'Kafedra topilmadi.', ik([[('⬅️ Sozlamalar','admin:settings')]]))
    st = department_stats(dkey)
    txt = f"🏛 <b>{safe(d['name'])}</b>\nHolat: <b>{'Faol' if d['is_active'] else 'Arxiv'}</b>\nO‘qituvchilar: <b>{st['teachers']}</b>\nBaholar: <b>{st['votes']}</b>\nFinal: <b>{st['final']:.3f}</b>"
    await edit_or_send(call, txt, ik([[('✏️ Tahrirlash',f'settings:depedit:{dkey}'),('🗄/✅ Holat',f'settings:deptoggle:{dkey}')],[('⬅️ Kafedralar sozlamasi','settings:deps'),('⬅️ Sozlamalar','admin:settings')]]))

@dp.callback_query(F.data == 'admin:clear')
@admin_required
async def admin_clear_menu(call: CallbackQuery):
    await answer_cb(call)
    await edit_or_send(call, '🧹 <b>Ma’lumotlarni tozalash</b>\n\nQaysi ma’lumotni tozalashni tanlang. Har bir bo‘lim alohida tozalanadi.', admin_clear_kb())

@dp.callback_query(F.data.startswith('clear:ask:'))
@admin_required
async def clear_ask(call: CallbackQuery):
    await answer_cb(call)
    target = call.data.split(':', 2)[2]
    item = CLEAR_TARGETS.get(target)
    if not item:
        return await edit_or_send(call, 'Noto‘g‘ri bo‘lim.', admin_clear_kb())
    title, table = item
    try:
        count = conn.execute(f'SELECT COUNT(*) FROM {table}').fetchone()[0]
    except Exception:
        count = 0
    await edit_or_send(call, f"⚠️ <b>{safe(title)}</b> bo‘limini tozalashni tasdiqlaysizmi?\n\nHozirgi yozuvlar soni: <b>{count}</b>\nBu amal qaytarilmaydi.", clear_confirm_kb(target))

@dp.callback_query(F.data.startswith('clear:do:'))
@admin_required
async def clear_do(call: CallbackQuery):
    await answer_cb(call, 'Tozalanmoqda...')
    target = call.data.split(':', 2)[2]
    item = CLEAR_TARGETS.get(target)
    if not item:
        return await edit_or_send(call, 'Noto‘g‘ri bo‘lim.', admin_clear_kb())
    ...
    try:
        ...
        return await edit_or_send(call, f'❌ Tozalashda xatolik: <code>{safe(str(e))}</code>', admin_clear_kb())
    await edit_or_send(call, f'✅ <b>{safe(title)}</b> tozalandi.', admin_clear_kb())
...
async def set_deps(call:CallbackQuery):
    await answer_cb(call)
    rows=[]
    for d in departments(active=False):
        rows.append([(f"{'✅' if d['is_active'] else '🗄'} {d['name']}"[:40], f"settings:dep:{d['department_key']}")])
    rows.append([('➕ Qo‘shish','settings:depadd')])
    rows.append([('⬅️ Sozlamalar','admin:settings')])
    await edit_or_send(call, '🏛 <b>Kafedralar sozlamasi</b>\nKafedrani tanlang yoki yangisini qo‘shing:', ik(rows))

@dp.callback_query(F.data.in_({'settings:complaints','set:complaints'}))
@admin_required
async def set_complaints(call:CallbackQuery):
    await answer_cb(call)
    await edit_or_send(call, '📢 <b>Shikoyatlar sozlamasi</b>\nShikoyatlar ro‘yxatini ko‘rish yoki Excel/backupdan foydalanish mumkin.', ik([[('📋 Shikoyatlar ro‘yxati','admin:complaints'),('📥 Excel','export:excel')],[('⬅️ Sozlamalar','admin:settings')]]))

@dp.callback_query(F.data.in_({'settings:rating','set:rating'}))
@admin_required
async def set_rating(call:CallbackQuery):
    await answer_cb(call)
    await edit_or_send(call, '📊 <b>Reyting sozlamasi</b>\nReytinglarni ko‘rish va export qilish:', ik([[('👨‍🏫 O‘qituvchilar','admin:rating:teachers'),('🏛 Kafedralar','admin:rating:departments')],[('⬅️ Sozlamalar','admin:settings')]]))

@dp.callback_query(F.data.in_({'settings:backup','set:backup'}))
@admin_required
...
@dp.callback_query(F.data.in_({'settings:admins','set:admins'}))
@admin_required
